...
class Processer:
    @classmethod 
    def process(cls, text: str, role_rules_path: str, morph_rules_path: str)->List[Dict[str, str]]:
        text = text.replace('\n', ' ')
...
res = Processer.process("""Lucas goes to school every day of the week. He has many subjects to go to each school day: English, art, science, mathematics, gym, and history. His mother packs a big backpack full of books and lunch for Lucas.
...
Then Lucas gets his break for lunch. He sits with Kyle while he eats. The principal, or the headmaster as some call him, likes to walk around and talk to students during lunch to check that they are all behaving.""", 'dep_rules.json', 'morph_rules.json')
